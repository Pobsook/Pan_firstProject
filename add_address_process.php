...
if(isset($_POST['shipping_info'])){
    if(!empty($_POST['name_or_company']) && !empty($_POST['id_card_or_passport']) && !empty($_POST['location_delivery']) && !empty($_POST['telephone'])){
        ...
        $_SESSION['shipping_info']['name_or_company'] = $name_or_company;
        $_SESSION['shipping_info']['telephone'] = $telephone;
        $_SESSION['shipping_info']['id_card_or_passport'] = $id_card_or_passport;
...
if(isset($_POST['tax_info'])){
    if(!empty($_POST['name_or_company_tax']) && !empty($_POST['id_card_or_passport_tax']) && !empty($_POST['address_tax_Invoice'])){
        ...
        if($insert_info -> execute()){
            $_SESSION['tax_info']['name_or_company_tax'] = $name_or_company_tax;
            $_SESSION['tax_info']['idCard_or_idCompany'] = $id_card_or_passport_tax;
            $_SESSION['tax_info']['address_tax'] = $address_tax_Invoice;
